...
import '../../../data/services/amplify_service.dart';
...
class _AIFeaturesScreenState extends ConsumerState<AIFeaturesScreen> {
  ...
  Future<void> _checkAmplifyStatus() async {
    setState(() {
      _isAmplifyConfigured = AmplifyService().isConfigured;
    });
  }
  ...
  Future<void> _configureAmplify() async {
    ...
    try {
      ...
      setState(() {
        ...
        _successMessage = 'Amplify 구성이 완료되었습니다!';
      });
    } catch (e) {
      setState(() {
        _errorMessage = 'Amplify 구성 실패: $e';
      });
    } finally {
      setState(() {
        _isLoading = false;
    ...
    return Column(
      ...
      children: [
        if (!_isAmplifyConfigured)
          ElevatedButton(
            onPressed: _isLoading ? null : _configureAmplify,
            child: _isLoading
                ? const SizedBox(
                    height: 20,
                    width: 20,
